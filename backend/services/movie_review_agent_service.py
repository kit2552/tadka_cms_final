...
{story_review_text}"""
    ...
    def _create_article_data(self, content_workflow: str, article_language: str, rewritten_sections: Dict) -> Dict:
        ...
        # Check if YouTube trailer URL exists
        youtube_url = self.temp_review_data.get('youtube_url', '') or ''
        
        # For movie reviews, the image should ONLY come from YouTube thumbnail
        # We don't use the scraped poster image directly
        # If YouTube URL exists, we'll generate the thumbnail URL from it
        
        # Determine if action is needed (missing YouTube trailer)
        action_needed = False
        ...
        # Map workflow to status and publish state
        # If action_needed, always set to approved but NOT published
        if action_needed:
            status = 'approved'
            is_published = False
            print(f"      ⚠️  Action Needed: {', '.join(action_needed_reasons)}")
            # No image without YouTube trailer
            poster_image = ''
        else:
            status_map = {
                'in_review': 'draft',
    ...
    async def _extract_pinkvilla_hindi_reviews(self, soup: BeautifulSoup, base_url: str, max_links: int) -> list:
        """
        Extract movie review URLs from Pinkvilla listing page
        No language filtering - all reviews are included
        """
        import json
        from urllib.parse import urlparse
            ...
            try:
                ...
                if isinstance(json_data, dict) and json_data.get('@type') == 'ItemList':
                    ...
                    for item in items:
                        ...
                        # Only include review URLs
                        if '/reviews/' in item_url.lower() and 'movie-review' in item_url.lower():
                            if item_url not in review_links:
                                name = item.get('name', 'Unknown')
                                review_links.append(item_url)
                                print(f"      ✅ Review: {name[:60]}...")
                
            except (json.JSONDecodeError, Exception) as e:
                continue
        
        # Method 2: Fallback to HTML link extraction if JSON-LD didn't find enough
        if len(review_links) < max_links:
            for link in soup.find_all('a', href=True):
                if len(review_links) >= max_links:
                    break
                ...
                # Check if it's a movie review URL
                if '/reviews/' in href.lower() and 'movie-review' in href.lower():
                    # Make absolute URL
                    if href.startswith('http'):
                        full_url = href
                    elif href.startswith('/'):
                    ...
                    # Avoid duplicates and pagination
                    if full_url not in review_links and '/page/' not in full_url.lower():
                        review_links.append(full_url)
                        print(f"      ✅ Review (HTML): {full_url.split('/')[-1][:50]}...")
