...
{story_review_text}"""
    ...
    def _create_article_data(self, content_workflow: str, article_language: str, rewritten_sections: Dict) -> Dict:
        ...
        poster_image = self.temp_review_data.get('poster_image', '') or ''
        
        # Determine if action is needed (missing YouTube trailer or poster image)
        action_needed = False
        ...
        if not poster_image:
            action_needed = True
            action_needed_reasons.append('Missing poster image')
        ...
        if action_needed:
            ...
            # Don't use the scraped image if no YouTube trailer
            if not youtube_url:
                poster_image = ''  # Clear the image
        else:
            status_map = {
                'in_review': 'draft',
