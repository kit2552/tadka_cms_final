...
@router.put("/items/{item_id}/mark-used")
async def mark_item_used(item_id: str):
    """Mark a feed item as used"""
    result = db[RELEASE_FEED_ITEMS].update_one(
        {"id": item_id},
...
async def skip_item(item_id: str):
    """Mark a feed item as skipped"""
    result = db[RELEASE_FEED_ITEMS].update_one(
        {"id": item_id},
        {"$set": {"is_skipped": True, "skipped_at": datetime.now(timezone.utc)}}
    )
...
async def delete_feed_item(item_id: str):
    ...
    result = db[RELEASE_FEED_ITEMS].delete_one({"id": item_id})
