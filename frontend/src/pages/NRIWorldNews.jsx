...
const NRIWorldNews = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('nri-news'); // 'nri-news' or 'world-news'
  const [nriNewsArticles, setNriNewsArticles] = useState([]);
  const [worldNewsArticles, setWorldNewsArticles] = useState([]);
  ...
  useEffect(() => {
    const savedScrollPosition = sessionStorage.getItem('nriWorldNewsScrollPosition');
  ...
  useEffect(() => {
    const fetchNRIWorldNewsData = async () => {
      try {
        setLoading(true);
        ...
        if (worldResponse.ok) {
          ...
          // Fallback: use nri news articles for world news if not available
          const nriData = await nriResponse.json();
          setWorldNewsArticles(nriData);
        }
        ...
        console.error('Error loading NRI & World News data:', err);
    ...
    fetchNRIWorldNewsData();
  }, []);

  // Update filtered articles when tab or filter changes
  useEffect(() => {
    const currentArticles = activeTab === 'world-news' ? worldNewsArticles : nriNewsArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [nriNewsArticles, worldNewsArticles, activeTab, selectedFilter]);
  ...
  const handleArticleClick = (article) => {
    ...
    sessionStorage.setItem('nriWorldNewsScrollPosition', window.scrollY.toString());
    ...
    if (article.content_type === 'video' || article.content_type === 'video_post') {
      navigate(`/video/${article.id}`, { state: { from: 'nri-world-news' } });
    } else {
      const slug = article.slug || article.title.toLowerCase().replace(/\s+/g, '-');
      navigate(`/article/${article.id}/${slug}`, { state: { from: 'nri-world-news' } });
    }
  };
    ...
    <>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* NRI & World News Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
              ...
              <div className="pl-0 pr-4 py-4">
                <div className="mb-2">
                  ...
                  <div className="flex space-x-4">
                    ...
                    <button
                      onClick={() => setActiveTab('world-news')}
                      className={`text-base font-bold transition-colors duration-200 ${
            ...
            {filteredArticles.length === 0 && (
              <div className="text-center py-8">
                <p className="text-sm text-gray-400 mb-1">No {activeTab === 'nri-news' ? 'NRI news' : 'world news'} articles found</p>
