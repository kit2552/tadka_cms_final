...
const CommentSection = ({ articleId, commentType = 'regular', headerTitle = 'Comments' }) => {
  ...
  return (
    <div>
      ...
      <div className={`sticky top-0 z-30 border-b-2 border-gray-300 mb-4 -mr-4`} style={{ backgroundColor: 'rgb(249 250 251 / var(--tw-bg-opacity, 1))' }}>
        <div className="pl-0 pr-0 py-4 flex items-center justify-between gap-2">
      ...
      {loading ? (
        ...
        <div className={`space-y-3 -mr-4 ${comments.length > 5 ? 'max-h-[450px] overflow-y-auto pr-2 custom-scrollbar' : ''}`}>
            ...
            >
              <div className="mb-2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <h4 className="font-semibold text-gray-900 text-xs">{comment.name}</h4>
                  {comment.rating && (
                    <div className="flex items-center gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <svg
                          key={i}
                          className={`w-3 h-3 ${i < parseInt(comment.rating) ? 'text-yellow-400' : 'text-gray-300'}`}
                          fill="currentColor"
                          viewBox="0 0 24 24"
                        >
              ...
              <p className="text-gray-700 text-xs whitespace-pre-wrap text-left">{comment.comment}</p>
            </div>
          ))}
        </div>
      )}
